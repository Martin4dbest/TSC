  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setStats({
          users: mobileAppUsers < 0 ? 0 : mobileAppUsers,
    ...
    <div className={`${theme} min-h-screen flex font-sans text-[11px]`}>
      ...
      <main className="ml-56 flex-1 p-6 lg:p-8 transition-all duration-200">
        <div className="flex justify-between items-center mb-6 pb-4 border-b border-slate-800/30">
          ...
          <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 tracking-wider uppercase">
            {status}
          </span>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <Card
            title="Live Tracking"
            desc="GPS + Movement Engine"
            cardStyle={cardStyle}
          />

          <Card
            title="Emergency System"
            desc="SOS + Response Network"
            cardStyle={cardStyle}
          />
...
function Nav({
  ...
  return (
    ...
    >
      <div className={active ? "text-blue-400" : color}>
        {icon}
      </div>
...
function Stat({
  ...
  return (
    <div
      className={`p-3.5 rounded-lg border transition-all duration-200 ${cardStyle}`}
    >
...
function Card({
  ...
  return (
    <div
      className={`p-4 rounded-lg border transition-all duration-200 cursor-pointer ${cardStyle}`}
    >
      <h3 className="font-semibold text-slate-200 mb-0.5 text-[11px] tracking-wide">
        {title}
      </h3>
